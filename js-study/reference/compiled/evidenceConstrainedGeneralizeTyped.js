...
exports.evidenceConstrainedDouglasPeuckerTyped = evidenceConstrainedDouglasPeuckerTyped;
/**
 ...
 */
const typedEvidenceOperators_1 = require("./typedEvidenceOperators");
...
function dpIndices(v, a, b, tol) {
    ...
    let idx = -1, max = -1;
    for (let i = a + 1; i < b; i++) {
        const d = perpendicularDistance(v[i], v[a], v[b]);
        if (d > max) {
            max = d;
            idx = i;
        }
 ...
 */
function mandatory(v, extra) {
    const keep = new Set([0, v.length - 1, ...extra]);
    for (let i = 0; i < v.length; i++) {
        if (v[i].state === 'unsupported')
            keep.add(i);
    }
    return [...keep].sort((a, b) => a - b);
}
function hasUnsupported(v, a, b) {
    for (let i = a; i <= b; i++)
        if (v[i].state === 'unsupported')
            return true;
    return false;
 ...
 * unavailable.
 */
function arcEvidence(v, a, b) {
    const inputs = [];
    for (let i = a; i <= b; i++) {
        const { x: _x, y: _y, state: _state, ...evidence } = v[i];
        inputs.push(evidence);
    }
    return (0, typedEvidenceOperators_1.typedMeet)(inputs);
}
function evidenceConstrainedDouglasPeuckerTyped(vertices, tolerance, extraProtected = new Set()) {
    if (!Number.isFinite(tolerance) || tolerance < 0)
        throw new RangeError('invalid tolerance');
    if (vertices.length < 2)
        return { retainedIndices: vertices.map((_, i) => i), segments: [] };
    vertices.forEach((x, i) => { if (!Number.isFinite(x.x) || !Number.isFinite(x.y))
        throw new RangeError(`invalid coordinate at ${i}`); });
    const m = mandatory(vertices, extraProtected), keep = new Set(m);
    for (let j = 0; j < m.length - 1; j++) {
        const a = m[j], b = m[j + 1];
        if (hasUnsupported(vertices, a, b)) {
            for (let i = a; i <= b; i++)
                keep.add(i);
            continue;
        }
        for (const i of dpIndices(vertices, a, b, tolerance))
            keep.add(i);
    }
    const retainedIndices = [...keep].sort((a, b) => a - b);
    const segments = [];
    for (let j = 0; j < retainedIndices.length - 1; j++) {
        const a = retainedIndices[j], b = retainedIndices[j + 1];
        if (hasUnsupported(vertices, a, b))
            continue;
