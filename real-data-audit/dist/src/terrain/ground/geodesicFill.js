...
exports.geodesicFill = geodesicFill;
 ...
 */
function geodesicFill(z, hadData, cols, rows, params = {}) {
    const n = cols * rows;
    const out = new Float32Array(n);
    out.set(z);
    if (n === 0)
        return out;
    const power = Number.isFinite(params.power) && params.power > 0 ? params.power : 2;
    ...
    const surface = (0, idwFill_1.idwFill)(z, hadData, cols, rows, { power, kNearest, maxRadiusCells: maxRadius });
    ...
    const heapPop = () => {
        ...
        if (heapLen > 0) {
            ...
            for (;;) {
                ...
                let s = i;
                if (l < heapLen && heapCost[l] < heapCost[s])
                    s = l;
                if (r < heapLen && heapCost[r] < heapCost[s])
                    s = r;
                if (s === i)
                    break;
                swap(s, i);
    ...
    for (let srow = 0; srow < rows; srow++) {
        for (let scol = 0; scol < cols; scol++) {
            ...
            if (hadData[src] === 1)
                continue; // measured — keep verbatim
            if (!Number.isFinite(surface[src])) {
                out[src] = Number.NaN;
                continue;
            } // unreachable gap
            ...
            while (heapLen > 0 && collected < kNearest) {
                ...
                for (let k = 0; k < 8; k++) {
                    ...
                    if (Math.abs(nr - srow) > maxRadius || Math.abs(nc - scol) > maxRadius)
                        continue;
                    const nb = nr * cols + nc;
                    if (!Number.isFinite(surface[nb]))
                        continue; // can't walk over unknown ground
                    let stepXY;
                    if (DR[k] !== 0 && DC[k] !== 0)
                        stepXY = cellDiag;
                    else if (DC[k] !== 0)
