...
exports.hornSlopeAspect = hornSlopeAspect;
exports.synthesisedNeighbourMask = synthesisedNeighbourMask;
exports.countSynthesisedNeighbours = countSynthesisedNeighbours;
exports.hornSlope = hornSlope;
/**
 ...
 */
function hornSlopeAspect(z, cols, rows, cellMetresX, cellMetresY = cellMetresX, zScale = 1) {
    const n = cols * rows;
    const slope = new Float32Array(n);
    const aspect = new Float32Array(n);
    if (n === 0 || !(cellMetresX > 0) || !(cellMetresY > 0))
        return { slope, aspect };
 ...
 */
function synthesisedNeighbourMask(z, cols, rows) {
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            ...
            if (!Number.isFinite(z[i]))
                continue; // no derivative to qualify
...
function countSynthesisedNeighbours(z, cols, rows) {
...
function hornSlope(z, cols, rows, cellMetresX, cellMetresY = cellMetresX, zScale = 1) {
